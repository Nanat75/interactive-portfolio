

<h1 align="center">🌸 Nathasya — Interactive Pixel Portfolio</h1>

<p align="center">
  <i>A soft pastel, game-like portfolio where curiosity is rewarded.</i>
</p>

<p align="center">
  🎮 Explore • 🔑 Collect • 🌿 Interact • 💌 Discover
</p>

---

## 🏡 About This Project

This is **not a normal portfolio website**.

It’s an **interactive pixel-art experience**, inspired by cozy games like  
**OMORI**, classic RPGs, and room-exploration mechanics.

Instead of scrolling, visitors:
- Walk around a room
- Interact with objects
- Unlock areas
- Discover information naturally ✨

The goal?  
👉 Make learning about me feel **personal, playful, and memorable!**.

---

## 🎮 Features

✨ Game-style navigation (keyboard-based)  
✨ Pixel-art environment & UI  
✨ Interactive objects (notes, shelves, doors, TV, diary. and more)  
✨ Locked-room mechanics (keys, Task, fragments)  
✨ Soft background music & sound effects  
✨ Exit choice system (leave or stay longer)  
✨ Pastel + cozy pixel aesthetic  

---

## 🗺️ Rooms & Interactions

| Room | Purpose |
|------|---------|
| 🚪 **Start Door** | Entry point & introduction to the portfolio |
| 🛋️ **Living Room** | Welcome note, Home section, and About Me |
| 🍰 **Kitchen** | Skills presented as a playful mini-game |
| 🧰 **Workshop** | Featured projects & experiments |
| 🌿 **Bed** | Contact section and exit choice |

>  Explore freely, interact with objects, and uncover content naturally.

---

### - Frontend
<p>
  <img src="https://skillicons.dev/icons?i=html,css,js" />
</p>

### - Design & Creative Tools
<p>
  <img src="https://skillicons.dev/icons?i=figma" />
  <img src="https://1000logos.net/wp-content/uploads/2023/02/Canva-logo.png" height="48"/>
  <img src="https://cdn.pixilart.com/admin/files/953a65916ff3b11.png" height="48"/>
</p>

---

## 🚀 Live Demo

🌐 **Play the portfolio here:**  
👉  https://nanat75.github.io/interactive-portfolio/

*(Best experienced on desktop with sound on 🎧)*

---

<p align="center">
  <i>Thanks for exploring my little pixel room 🌷</i><br>
  <i>Feel free to stay… or come back anytime.</i>
</p>

---
